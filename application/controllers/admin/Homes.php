...
class Homes extends MY_Controller {
    ...
    public function index() {
        ...
        $listJs = array(
            ...
            '/static/admin/js/extra/homes.js'
        );
    ...
    public function remove() {
        ...
        if(!empty($id)){
            ...
            $result = $this->db->where_in('id', explode(',', $id))->update($this->sliderModel, $pullClass);
...
//        $permission = $this->check_permission($this->class, 'view');
        ...
        if(!empty($id)){
            $item = $this->db->select()->from($this->sliderModel)->where('id', $id)->where('deleted', 0)->get()->row();
            $response->status = 1;
            $response->item = $item;
            $this->response($response);
        }
        ...
        $query = $this->db->select('`id`, `kind` k, `title` t, `sub` s, `background` b, `sort` so')->from($this->sliderModel)->where('deleted', 0);
        $this->criteria($query);
        ...
        if($num){
            ...
            $all = $this->db->select('id')->from($this->sliderModel)->where('deleted', 0)->get()->num_rows();
        }
        $response->data = $set;
    ...
    function save(){
        ...
        if(!empty($kind)){
            ...
            $query = /** @lang text */
                "
                INSERT INTO `".$this->sliderModel."` (".(!empty($id) ? '`id`, ' : '')."`kind`,
