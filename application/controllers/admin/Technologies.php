...
class Technologies extends MY_Controller {
	...
	function __construct() {
        ...
        $this->designModel = 'sys_technologies';
        $this->viewPath = 'admin/technologies/';
    }
    ...
    public function index() {
        ...
        $listJs = array(
            ...
            '/static/admin/js/extra/designs.js'
        );
    ...
    public function remove() {
        ...
        if(!empty($id)){
            ...
            $result = $this->db->where_in('id', explode(',', $id))->update($this->designModel, $pullClass);
...
//        $permission = $this->check_permission($this->class, 'view');
        ...
        if(!empty($id)){
            $item = $this->db->select()->from($this->designModel)->where('id', $id)->where('deleted', 0)->get()->row();
            $response->status = 1;
            $response->item = $item;
            $this->response($response);
        }
        ...
        $sortMaps = array('sort', 'id', 'kind', 'title', 'sub', 'background', 'sort');
        $order = $this->input->post('order', true);
        $sort = isset($order[0]['column']) ? $order[0]['column'] : 0;
        $type = isset($order[0]['dir']) ? $order[0]['dir'] : 'desc';
        $start = $this->input->post('start', true);
        $query = $this->db->select('`id`, `kind` k, `title` t, `sub` s, `background` b, `sort` so')->from($this->designModel)->where('deleted', 0);
        $this->criteria($query);
        ...
        if($num){
            ...
            foreach($list as $index=>$item){
                array_push($set, array(null, $item->id, $item->k, $item->t, $item->s, $item->b, $item->so));
            }
            $all = $this->db->select('id')->from($this->designModel)->where('deleted', 0)->get()->num_rows();
        }
        $response->data = $set;
    ...
    function save(){
        ...
        if(!empty($kind)){
            ...
            $query = /** @lang text */
                "
                INSERT INTO `".$this->designModel."` (".(!empty($id) ? '`id`, ' : '')."`kind`,
